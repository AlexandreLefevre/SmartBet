import React,  {Component} from 'react'
import{
   Link 
 }  from 'react-router-dom'
 import { Row, Col } from 'antd';

class Footer extends Component {
  render() {
     return (
      <footer class="footer-distributed">
         <Row gutter={[8, 8]}>
            <Col span={8}> 
               <h2>Riv<a href='/cache' class="gris">4</a>l<span>Gaming</span></h2>
               <Link to="/">Acceuil</Link>         
               <Link to="/forum">Forum</Link>        
               <Link to="/tournois">Tournois</Link>           
               <Link to="/faq">Faq</Link>           
               <p class="footer-company-name">Riv4lGaming © 2020</p> </Col>
            <Col span={8}>               
               <p><span>Rue ciseaux</span> Louvain La Neuve, Belgique</p>               
               <p>0470.34.16.01</p> 
               <p><a href="mailto:support@company.com">support@Riv4lGaming.com</a></p>
            </Col>
            <Col span={8}>
               <span>A propos</span>
               <p>Riv4lGaming est un projet étudiant qui à pour objectif de créer un site web sur le thème des tournois de jeux vidéos.</p>
                  <a href="https://www.facebook.com/"><i class="fa fa-facebook"><img style={{width:"100px", height:"100px"}} src="data:image/png;base64,"></img></i></a>
                  <a href="https://twitter.com"><i class="fa fa-twitter"><img style={{width:"100px", height:"100px"}} src="https://encrypted-tbn0.gstatic.com/images?q=tbn%3AANd9GcTDE0n9vCfD2ilPR0d8iAnalumrfMME2muC5vUhzGjSB-WLJnEn"></img></i></a>
                  <a href="https://fr.linkedin.com/"><i class="fa fa-linkedin"><img style={{width:"100px", height:"100px"}} src="https://encrypted-tbn0.gstatic.com/images?q=tbn%3AANd9GcSQQiI56jFu1b6Pu_oIrLS3QHrS9xBmQXmnTdCALGgp1A115Q4i"></img></i></a>
                  <a href="https://github.com/AlexandreLefevre/SmartBet"><i class="fa fa-github"><img style={{width:"100px", height:"100px"}} src="data:image/png;base64,"></img></i></a>
            </Col>
         </Row>
   </footer>
     );
  }
}

export default Footer;